...
> **说明：** Node 后端完整模板已迁移至 [node_koa_template.md](node_koa_template.md)。生成代码时请以 **node_koa_template.md** 为准，保证登录流程稳定。本文件保留兼容，内容与 node_koa_template.md 一致。
...
router.get('/oauth/login', async (ctx) => {
    try {
        ...
        if (!clientId || !redirectUri || clientId === '[从Step 1获取的data.id]' || redirectUri === '[从Step 1获取的data.redirectUris]') {
            ctx.logger.error('[OAuth2] 配置缺失：CLIENT_ID 或 REDIRECT_URI 未设置，请检查 server/config/index.js 中常量是否已从 Step 1 回填');
            ctx.status = 500;
            ctx.body = 'OAuth2 配置缺失：请检查 server/config/index.js 中常量 OAUTH2_CLIENT_ID、OAUTH2_REDIRECT_URI 是否已从 Step 1 回填';
            return;
        }
        
        // 构建授权 URL（禁止添加scope参数）
        const params = {
            client_id: clientId,
...
router.get('/callback', async (ctx) => {
    ...
    try {
        ...
        // ⚠️ 关键：获取重定向URL，并确保使用配置的IP地址而不是localhost
        // 优先级：1) OAuth服务器返回的path参数 2) 内存中保存的path 3) session中的redirectAfterLogin 4) 默认'/'
        let redirectUrl = redirectPath || savedPath || ctx.session?.redirectAfterLogin || '/';
        
        // 修复重定向URL，确保使用配置的IP地址
...
```env
# OAUTH2_CLIENT_ID、OAUTH2_CLIENT_SECRET、OAUTH2_REDIRECT_URI 已在 server/config/index.js 中常量存储，不在此处配置
