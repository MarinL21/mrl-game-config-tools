...
def runtime_srcs(src):
    """headless Chrome 跑一遍拿最终 DOM —— 运行时 JS 拼出来的 <img src> 静态正则扫不全
    （`${MB}弹珠_${x}.png` 这类带动态段的路径），只有渲染后的 DOM 才是完整清单。"""
    if not os.path.isfile(CHROME):
        return []
    r = subprocess.run([CHROME, "--headless", "--disable-gpu", "--no-sandbox",
                        "--allow-file-access-from-files", "--virtual-time-budget=3000",
                        "--dump-dom", f"file://{src}"], capture_output=True)
    dom = r.stdout.decode("utf-8", "replace")
    return sorted(set(re.findall(r'src="(assets/[^"]+\.(?:png|jpg|jpeg|webp|gif|svg))"', dom)))
...
def bundle(src):
    ...
    # 5) \u8fd0\u884c\u65f6\u8d44\u6e90\u515c\u5e95 \u2014\u2014 4) \u8986\u76d6\u4e0d\u5230\u300c\u524d\u7f00\u5e38\u91cf + \u52a8\u6001\u6bb5\u300d\u7684\u8def\u5f84\uff08`${MB}\u5706\u94ae_${\u8272}.png`\uff09\u3002
    #    \u8dd1\u771f\u5b9e DOM \u62ff\u5b8c\u6574 src \u6e05\u5355\uff0c\u6ce8\u5165\u53bb\u91cd\u7684\u8d44\u6e90\u6620\u5c04 + \u5efa DOM \u540e\u7edf\u4e00\u6539\u5199 <img>\u3002
    #    \u6620\u5c04\u6bcf\u4e2a\u6587\u4ef6\u53ea\u5b58\u4e00\u4efd data URI\uff0c\u6bd4\u628a src \u9010\u4e2a\u5c55\u5f00\u5c0f\u5f97\u591a\u3002
    rt = [p for p in runtime_srcs(src) if local(p)]
    if rt:
        amap = ",\n".join(f'"{p}":"{data_uri(local(p))}"' for p in rt)
        stats["img"] += len(rt)
        html += ('\n<script>\nwindow.__ASSET__ = {\n' + amap + '\n};\n'
                 '(function () {\n'
                 '  var M = window.__ASSET__;\n'
