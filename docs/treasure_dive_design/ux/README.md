...
https://rawcdn.githack.com/MarinL21/mrl-game-config-tools/dc07408/docs/treasure_dive_design/ux/intro3.preview.html
...
> githack 链接里那段 `dc07408` 是 commit sha，**换稿后要换成新 sha** 才是新版本（sha 版永久缓存、不会串版）。
...
| 页 | 三联 | 底部横条 |
|---|---|---|
| ① 怎么下潜 | 点开格子 → 挨着空格才能点 → 会一直往下潜 | 5 类格子各花多少水母 |
...
## 下潜口径（0909 按案子「下潜示意」纠过一次）

**底行有空格 → 往下潜一行（+10 米）；新补的一行还有空格 → 接着潜；直到有一行没有空格才停。**
⛔ 别写成「挖穿一行才下潜」；可点条件也别用「通路 / 绕开」这种行话（用户看不懂），说「挨着空格才能点，挖开上面就够得着」—— 停止条件是「某一行没有空格」，这才是案子里的口径。
单次最多连潜 32 行是服务端 `MaxDivePerAction` 保险丝，兜底用的，不进玩家文案。
