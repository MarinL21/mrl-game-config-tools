...
https://rawcdn.githack.com/MarinL21/mrl-game-config-tools/735b52b/docs/treasure_dive_design/ux/intro3.preview.html
...
> githack 链接里那段 `735b52b` 是 commit sha，**换稿后要换成新 sha** 才是新版本（sha 版永久缓存、不会串版）。
...
| ③ 海螺换奖 + 深度奖励 | 海螺从哪来 → 拿海螺换奖 → 潜得越深奖越大 | 深度玩法的四条功能口径 |

**能点的**：左右翻页圆钮、底部页码点、右上 ✕（关闭 → 「再看一次」叫回来）、③页深度轨（点一档看「未达成 / 可领取」两态）。三联分镜本身是**静态示意**——实装每联各 K 一小段循环动画，别摆死图。

⛔ **这三页只讲功能，不讲奖励详情**（用户 0909 口径：功能介绍别本末倒置，这页是为了让玩家玩懂）。
兑换商店只说「去换奖励」不列货位；深度奖励只说「有 11 档、越深越厚、12000 米大奖」不列每档内容 ——
货架和每档奖励玩家在活动界面自己看，配置口径写在板下的标注层里。
